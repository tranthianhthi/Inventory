...
namespace COGInterfaceCommand.Common.COG.ASN
{
    public class D_SKU : ICOGItem
    {
        ...
        public string checkSkuError(D_SKU sku)
        {
            ...
            float rp;
            string skuerrormess = "";
            if (ASN_Number == "") skuerrormess = skuerrormess + "ASN Number can not be null.";
            if (Brand == "") skuerrormess = skuerrormess + "Brand can not be null.";
            if (Colour == "") skuerrormess = skuerrormess + "Colour can not be null.";
            if (SizeDesc == "") skuerrormess = skuerrormess + "SizeDesc can not be null.";
            if (Style == "") skuerrormess = skuerrormess + "Style can not be null.";
            if (license_plate_ref == "") skuerrormess = skuerrormess + "license_plate_ref can not be null.";
            if (origin_country_code == "") skuerrormess = skuerrormess + "origin_country_code can not be null.";
            if (product_UOM == "") skuerrormess = skuerrormess + "product_UOM can not be null.";
            if (!float.TryParse(RRP.ToString(), out rp)) skuerrormess = skuerrormess + "RRP can not be null.";
            if (!int.TryParse(shipped_qty.ToString(), out qty)) skuerrormess = skuerrormess + "shipped_qty can not be null.";
            if (!int.TryParse(seq_number.ToString(), out qty)) skuerrormess = skuerrormess + "seq_number can not be null.";
            if (!int.TryParse(seq_number.ToString(), out qty)) skuerrormess = skuerrormess + "seq_number can not be null.";
            if (order_number == "") skuerrormess = skuerrormess + "order_number can not be null.";
            if (COGFileName == "") skuerrormess = skuerrormess + "COGFileName can not be null.";
            return skuerrormess;
        }
    }
}