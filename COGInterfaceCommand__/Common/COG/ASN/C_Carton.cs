...
namespace COGInterfaceCommand.Common.COG.ASN
{
    public class C_Carton : ICOGItem 
    {
        ...
        public string checkCartonError(C_Carton carton)
        {
            ...
            if (ASN_Number == "") cartonerrormess = cartonerrormess + "ASN Number can not be null.";
            if (license_plate_ref == "") cartonerrormess = cartonerrormess + "license_plate_ref can not be null.";
            if (order_number == "") cartonerrormess = cartonerrormess + "order_number can not be null.";
            if (!int.TryParse(seq_number.ToString(), out qty)) cartonerrormess = cartonerrormess + "seq_number can not be null.";
            if (COGFileName == "") cartonerrormess = cartonerrormess + "COGFileName can not be null.";
