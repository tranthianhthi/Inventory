...
namespace COGInterfaceCommand.Common
{
    public class InfologCOGCarton
    {
        ...
        private static string QueryConsignmentStatus = "SELECT DISTINCT consignment_number FROM Infolog_COG_Carton WHERE Sent = 0 AND Slip_no = 'S{0}' "; //Sent = 0 AND
        ...
        public static DateTime? GetLastConfirmShipmentDate()
        {
            ...
            try
            {
                ...
                date = tb.Rows[0]["LastDate"].Equals(DBNull.Value)? null: (DateTime?)tb.Rows[0]["LastDate"];
            }
            catch { throw; }
            return date;
        }
        ...
        public bool SaveToRetailPro(OracleTransaction trans)
        {
            ...
            try
            {
                ...
                int result = config.ExecuteRPCommand(config.RPConnection, command, paras);
                id = int.Parse(paras[0].Value.ToString());
