...
namespace COGInterfaceCommand.Command
{
    public class APICommand
    {
        ...
        public async void SubmitAPIToCOG(string uri, string message)
        {
            try
            {
                ...
                using (var content = new ByteArrayContent(byteData))
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ResponseContent = "Send Transfer Receipt via API." + Environment.NewLine + responseMsg;
                    }
                    else
                    {
                        ResponseContent = "Cannot Send Transfer Receipt via API." + Environment.NewLine + responseMsg;
                    }
                }
            }
            catch
        ...
        //public async void PostDataToCOG(string uri, object obj)
        //{
        //    try
        //    {
        //        client.DefaultRequestHeaders.Clear();
        ...
        //        string filename = Environment.CurrentDirectory + "\\Uploads\\FileJSON\\BackToCot-" + DateTime.Now.ToString("mm-dd-yyyy-HH-mm-ss") + ".json";
        ...
        public async void PatchDataToCOG(string uri, object obj, bool sendDCCode = false)
        {
            try
            {
                ...
                client.DefaultRequestHeaders.Clear();
                // Request headers if we call before no need call here.
                client.DefaultRequestHeaders.Add("Authorization", "" + Token); // Bearer Token       
                client.DefaultRequestHeaders.Add("Ocp-Apim-Subscription-Key", PrimarySubcriptionKey); // Subcription Key
                if (sendDCCode)
                {
                    client.DefaultRequestHeaders.Add("DC-Code", "0120");
