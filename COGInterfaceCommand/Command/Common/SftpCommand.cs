...
namespace COGInterfaceCommand.Command
{
    public class SftpCommand : InterfaceCommand
    {
        ...
        public string PutFileToSFTPServer(string remoteFolder,int salerecon)
        
        {
            ...
            if (salerecon == 1) { LocalUploadFolder = LocalUploadFolder + "\\UploadReconSale\\"; }
            ...
            using (Session session = new Session())
            {
                ...
                try
                {
                    ...
                    transferResult = session.PutFiles(LocalUploadFolder + "\\*.txt", remoteFolder, false);

                    transferResult.Check();

                    if (salerecon == 1) { LocalUploadFolder = Environment.CurrentDirectory + "\\FileUpload"; }
